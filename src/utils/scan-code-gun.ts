...
export class ScanCodeGunManager {
  ...
  private continuousScanTimeoutId?: number;
  ...
  constructor(config: Partial<ScanCodeGunConfig> = {}) {
    ...
    this.currentAdapter = this.adapters.get(this.config.type) || this.adapters.get('default')!;
  }
  ...
  private detectScannerInput(key: string): boolean {
    ...
    // 记录按键间隔
    if (interval > 0 && interval < 1000) { // 忽略异常大的间隔
  ...
  private analyzeScannerPattern(): boolean {
    ...
    if (this.scannerDetectionBuffer.length < this.config.minDataLength! ||
    ...
    if (this.keyIntervals.length >= 2) {
      ...
      const validIntervals = this.keyIntervals.filter(interval => interval < 100);
      ...
      if (avgInterval < this.config.minCharInterval! || avgInterval > this.config.maxCharInterval!) {
        return false;
      }
    ...
    const fastInputs = this.keyIntervals.filter(interval => interval < 10).length;
    ...
    // 2. 检查数据长度和输入速度的组合
    if (this.scannerDetectionBuffer.length >= 5 && this.keyIntervals.length >= 3) {
      const avgInterval = this.keyIntervals.reduce((sum, interval) => sum + interval, 0) / this.keyIntervals.length;
  ...
  private processScannedData(): void {
    ...
    try {
      ...
      const processedData = this.currentAdapter.processData(rawData, this.config);
      const isValid = this.currentAdapter.validateData(processedData, this.config);
      ...
      const scanData: ScanCodeData = {
        ...
        timestamp: Date.now(),
        isValid,
        error: isValid ? undefined : this.currentAdapter.getErrorMessage(processedData, this.config)
      };
  ...
  private handleContinuousScan(scanData: ScanCodeData): void {
    ...
    // 检查是否在连续扫码时间窗口内
    if (currentTime - this.lastScanTime <= this.config.continuousScanInterval!) {
      ...
      if (this.continuousScanCount >= this.config.maxContinuousScans!) {
        if (this.config.debug) {
          console.log('ScanCodeGun: Max continuous scans reached, calling completion callback');
        }

        // 调用连续扫码完成回调
        if (this.config.onContinuousScanComplete) {
          try {
            this.config.onContinuousScanComplete(this.continuousScanCount, [...this.continuousScanDataList]);
          } catch (error) {
            if (this.config.debug) {
              console.error('ScanCodeGun: Error in onContinuousScanComplete callback', error);
            }
          }
        }
    ...
    this.continuousScanTimeoutId = setTimeout(() => {
      if (this.config.debug) {
        console.log('ScanCodeGun: Continuous scan timeout, calling completion callback');
      }

      // 调用连续扫码完成回调
      if (this.config.onContinuousScanComplete && this.continuousScanDataList.length > 0) {
        try {
          this.config.onContinuousScanComplete(this.continuousScanCount, [...this.continuousScanDataList]);
        } catch (error) {
          if (this.config.debug) {
