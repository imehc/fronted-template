...
import { ScanCodeGunManager, createScanCodeGun, type ScanCodeGunConfig } from '../scan-code-gun';
...
describe('ScanCodeGunManager', () => {
  ...
  describe('基本功能', () => {
    ...
    it('应该能够更新配置', () => {
      ...
      const config = manager.getConfig();
      expect(config.autoEnter).toBe(false);
      expect(config.type).toBe('honeywell');
  ...
  describe('不同扫码枪适配器', () => {
    it('应该使用Honeywell适配器', (done) => {
      manager.updateConfig({ type: 'honeywell' });

      manager.addListener((data) => {
        expect(data.gunType).toBe('honeywell');
        done();
      });
    ...
    it('应该使用Datalogic适配器', (done) => {
      manager.updateConfig({ type: 'datalogic' });

      manager.addListener((data) => {
        expect(data.gunType).toBe('datalogic');
        done();
      });
    ...
    it('应该使用Zebra适配器', (done) => {
      manager.updateConfig({ type: 'zebra' });

      manager.addListener((data) => {
        expect(data.gunType).toBe('zebra');
        done();
      });
  ...
  describe('工具函数', () => {
    it('createScanCodeGun应该创建新的管理器实例', () => {
      const customManager = createScanCodeGun({
        autoEnter: false,
        type: 'honeywell'
      });
